...
\ \ \'/   \ \  __ \  \ \ \/\ \ \ \  __<   \ \ \  \ \  __\ \/_/\ \/ \ \___  \  
 ...
 If you're tryna do anything API related, it's specifically designed for vadrifts use. meaning it wouldnt work for you. ha ha.
]]
return function()
...
local function getTokenFromServer()
    local response = httpRequest({
        Url = "https://server2-78hd.onrender.com/generate?json=1",
        Method = "GET"
    })
...
local function sendHWIDToServer(token, hwid)
    httpRequest({
        Url = "https://server2-78hd.onrender.com/sethwid?token=" .. token .. "&hwid=" .. hwid,
        Method = "GET"
    })
end

local function openKeyLink(token)
    setclipboard("https://server2-78hd.onrender.com/key.html?token=" .. token)
    print("📋 Key URL copied to clipboard. Paste it in your browser.")
end
