...
if success then
    ...
    error("❌ Failed to get token from /generate")
end
...
if success2 then
    local keyData = HttpService:JSONDecode(keyResponse)
    key = keyData.key
    print("✅ Your Key:", key)
else
...
local function saveKey(key)
    ...
    local data = {
        key = key,
        hwid = hwid,
        hash = secureHash(key .. hwid .. KeySystemConfig.Title),
            ...
            for _, file in ipairs(files) do
                if string.match(file, "%.txt$") then
                    ...
                    if success then
                        ...
                        if success2 and data.key and data.hash and data.hwid then
                            local currentHwid = getHWID()
                            if data.hwid == currentHwid and data.hash == secureHash(data.key .. data.hwid .. data.scriptName) then
...
local function verifyKey(inputKey)
    ...
    if KeySystemConfig.UseKeyApi and KeySystemConfig.KeyApiUrl ~= "" then
        ...
        local combined = hwid .. weekString .. secret
        local hash = 0
        for i = 1, #combined do
...
function KeySystemAPI:Configure(config)
    ...
    if config.ActionLink then
        local hwid = getHWID()
        if hwid ~= "" and not string.find(config.ActionLink, "hwid=") then
            local separator = string.find(config.ActionLink, "?") and "&" or "?"
            KeySystemConfig.ActionLink = config.ActionLink .. separator .. "hwid=" .. hwid
        else
