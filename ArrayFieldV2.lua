...
if Settings.Discord then
    ...
    if not isfile(inviteFilePath) then
        local discordInviteUrl = "https://discord.gg/" .. Settings.Discord.Invite

        local success, err = pcall(function()
            game.openURL(discordInviteUrl)
        end)

        if success then
            if ArrayFieldLibrary and ArrayFieldLibrary.Notify then
                ArrayFieldLibrary:Notify({
                    Title = "Discord Invite",
                    Content = "Opening Discord invite: " .. discordInviteUrl .. "\nIf it doesn't open, copy the link from the console."
                })
            else
                warn("ArrayField: Opened Discord invite: " .. discordInviteUrl)
                print("ArrayField: Please check your browser for the Discord invite. If it didn't open, copy and paste this link: " .. discordInviteUrl)
            end
        else
            if ArrayFieldLibrary and ArrayFieldLibrary.Notify then
                ArrayFieldLibrary:Notify({
                    Title = "Discord Invite Failed",
                    Content = "Failed to automatically open Discord invite. Please manually join using: " .. discordInviteUrl
                })
            else
                warn("ArrayField: Failed to open Discord invite automatically. Error: " .. tostring(err))
                print("ArrayField: To join our Discord, please copy and paste this link into your web browser: " .. discordInviteUrl)
            end
        end
