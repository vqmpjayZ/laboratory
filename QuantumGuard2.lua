...
return function()
    ...
    local function checkFunction(funcName)
        return _G[funcName] or getgenv()[funcName] or (getfenv and getfenv()[funcName])
    end
    ...
    local function isfolderCompat(path)
        return safeCall(isfolder, path) or 
               safeCall(checkFunction("isfolder"), path) or 
               false
    end
    
    local function makefolderCompat(path)
        return safeCall(makefolder, path) or 
               safeCall(checkFunction("makefolder"), path)
    end
    
    local function writefileCompat(path, content)
        return safeCall(writefile, path, content) or 
               safeCall(checkFunction("writefile"), path, content)
    end
    
    local function readfileCompat(path)
        return safeCall(readfile, path) or 
               safeCall(checkFunction("readfile"), path)
    end
    
    local function listfilesCompat(path)
        return safeCall(listfiles, path) or 
               safeCall(checkFunction("listfiles"), path) or 
               {}
    end
    
    local function delfileCompat(path)
        return safeCall(delfile, path) or 
               safeCall(checkFunction("delfile"), path)
    end
    
    local function setclipboardCompat(text)
        return safeCall(setclipboard, text) or 
               safeCall(checkFunction("setclipboard"), text) or
               safeCall(toclipboard, text) or
               safeCall(checkFunction("toclipboard"), text)
    end

    local function getGuiParent()

        if safeCall(gethui) then
            return gethui()
        elseif safeCall(checkFunction("gethui")) then
            return checkFunction("gethui")()
    ...
    local function protectGui(gui)
        ...
        elseif checkFunction("protect_gui") then
            safeCall(checkFunction("protect_gui"), gui)
        end
    end
    ...
    local function getUniqueIdentifier()
        ...
        if hwid == "" then
            safeCall(function()
                hwid = checkFunction("gethwid") and checkFunction("gethwid")() or ""
            end)
        end
        
        if hwid == "" then
            hwid = "fallback_" .. tostring(player.UserId)
        end
    ...
    local function ensureFolderExists()
        safeCall(function()
            if not isfolderCompat("QuantumGuard") then
                makefolderCompat("QuantumGuard")
            end
        end)
    end
    ...
    local function saveKey(key)
        ...
        safeCall(function()
            ...
            writefileCompat(fileName, jsonData)
        end)
    end
    ...
    local function getSavedKeys()
        ...
        safeCall(function()
            if isfolderCompat("QuantumGuard") then
                local files = listfilesCompat("QuantumGuard")
                
                for _, file in ipairs(files) do
                    if string.match(file, "%.txt$") then
                        local fileContent = readfileCompat(file)
                        
                        if fileContent then
                            local success, data = safeCall(HttpService.JSONDecode, HttpService, fileContent)
                            
                            if success and data and data.key and data.hash then
                                local identifier = getUniqueIdentifier()
    ...
    keySystemGui.Name = "KeySystemGui_" .. tostring(math.random(1000, 9999))
    ...
    local function createRipple(button)
        ...
        safeCall(function()
            ...
            mousePos = mouse - Vector2.new(button.AbsolutePosition.X, button.AbsolutePosition.Y)
        end)
    ...
    actionText.MouseButton1Click:Connect(function()
        local success = setclipboardCompat(KeySystemConfig.ActionLink)
        ...
        if success then
            ...
            statusText.Text = "Failed to copy link!"
            statusText.TextColor3 = Color3.fromRGB(255, 100, 100)
        end
        
        spawn(function()
            (task and task.wait or wait)(3)
            statusText.Text = ""
        end)
    end)
    ...
    local function onVerifyClick()
        ...
        spawn(function()
            (task and task.wait or wait)(math.random(0.5, 1.5))
            ...
            if verifyKey(inputKey) then
                ...
                spawn(function()
                    for i = 1, 5 do
                        ...
                        (task and task.wait or wait)(0.05)
                    end
...
keyInput.FocusLost:Connect(function(enterPressed)
    ...
    spawn(function()
        (task and task.wait or wait)(0.1)
    ...
    if validSavedKey and KeySystemConfig.SaveKey then
        ...
        spawn(function()
            (task and task.wait or wait)(0.5)
            onVerifyClick()
        end)
    end
    ...
    function KeySystemAPI:GetExecutorInfo()
        local info = {
            name = "Unknown",
            hasFileSystem = false,
            hasClipboard = false,
        ...
        if syn then
            info.name = "Synapse X"
            info.hasProtection = syn.protect_gui ~= nil
        ...
        info.hasFileSystem = isfolder ~= nil or checkFunction("isfolder") ~= nil
        info.hasClipboard = setclipboard ~= nil or checkFunction("setclipboard") ~= nil or toclipboard ~= nil
        info.hasProtection = syn and syn.protect_gui or checkFunction("protect_gui") ~= nil
        
        return info
    end
    
    function KeySystemAPI:SetFallbackMode(enabled)
        if enabled then
            KeySystemConfig.SaveKey = false
            statusText.Text = "Running in compatibility mode"
            statusText.TextColor3 = Color3.fromRGB(255, 200, 100)
        end
        return self
    end

    local executorInfo = KeySystemAPI:GetExecutorInfo()
    if not executorInfo.hasFileSystem then
        KeySystemAPI:SetFallbackMode(true)
    end
