...
local function hideButton(BlurFrame)
    if not isVisible then return end
    isVisible = false
...
RunService.Heartbeat:Connect(function()
    if Hidden then
        showButton(BlurFrame)
        updateIcon(true)
    else
        hideButton(BlurFrame)
        updateIcon(false)
    end
end)
...
local isVisible = false
local isAnimating = false
local currentIcon = "rbxassetid://7734053495"
local hiddenIcon = "rbxassetid://7734053495"
local visibleIcon = "rbxassetid://7734058554"
...
local function createButton()
    ...
    ButtonFrame.Position = UDim2.new(1, -80, 0.5, -30)
    ...
    IconLabel.BackgroundTransparency = 1
    IconLabel.Image = currentIcon
    IconLabel.ImageColor3 = Color3.fromRGB(200, 200, 220)
    IconLabel.Parent = ButtonFrame
...
local function animateHover(hovering)
    local targetColor = hovering and Color3.fromRGB(35, 35, 45) or Color3.fromRGB(25, 25, 35)
    local targetStroke = hovering and Color3.fromRGB(60, 60, 80) or Color3.fromRGB(40, 40, 50)
...
local function updateIcon(hidden)
    currentIcon = hidden and hiddenIcon or visibleIcon
    IconLabel.Image = currentIcon
end
...
local function showButton()
    ...
    ButtonFrame.Position = UDim2.new(1, 0, 0.5, -30)
    ButtonFrame.Visible = true
...
local function simulateKeyPress()
    ...
    for _, connection in pairs(getconnections(UserInputService.InputBegan)) do
        connection.Function(virtualInput, false)
    end
end

local function init()
    local ClickDetector = createButton()
    ...
    UserInputService.TouchTapInWorld:Connect(function(position, processed)
        ...
        if not raycastResult then
            local screenPos = camera:WorldToScreenPoint(unitRay.Origin + unitRay.Direction * 10)
            local buttonPos = ButtonFrame.AbsolutePosition
            local buttonSize = ButtonFrame.AbsoluteSize
    ...
    local function checkRayfieldState()
        local rayfield = PlayerGui:FindFirstChild("Rayfield")
        if rayfield then
    ...
    RunService.Heartbeat:Connect(checkRayfieldState)
